<div id="app">
  ...
  <div class="exercise-list">
    <h2>Øvelser</h2>
    <ul>
      <li each={exercise in exercises} key={exercise.id}>
        {exercise.name} ({exercise.duration} sekunder)
      </li>
    </ul>
  </div>
  <div class="controls">
    <button on:click={startWorkout} disabled={isWorkoutRunning}>Start treningsøkt</button>
    <button on:click={stopWorkout} disabled={!isWorkoutRunning}>Stopp treningsøkt</button>
  </div>
</div>
